...
export default class productControllers {
    ...
    addProduct = async (req,res) => {
        ...
        if (!name || !description ||!container ||!brand || !price || !code || !stock || !category || !liters)res.status(400).send ("All fields are required")
            ...
            try {
                let validate = await productService.findbyAttribute("code",code)
    ...
    showProducts = async (req , res)=>{
        ...
        try{
             let result = await productService.paginate(filter, filterOptions)
             let io = req.app.get('socketio')
     ...
     updateProducts = async (req,res)=>{
        ...
        try {
            let result = await productService.update(id,update)
